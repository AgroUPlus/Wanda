...
/**
 * Melodic contours, for humming a tune at the app instead of naming it.
 *
 * No backfill, as with the acoustic vectors: a contour comes out of decoding the audio, so
 * existing libraries acquire them the next time the indexer runs. An empty table means engine B
 * finds nothing and the landmark engine answers alone, which is what happened before this existed.
 */
...
/**
 * Adds `track_lyrics` table and `lyrics_fts` full-text search virtual table for instant,
 * sub-millisecond lyrics search and timestamp line alignment.
 */
...
/**
 * Adds `downloadedAt`: when the downloader last wrote a track's local copy.
 *
 * Needed to tell a download nobody has touched in months from one just made — see
 * [com.wander.android.data.repository.StorageButlerRepository]. Existing downloaded rows get null
 * rather than a backfilled guess: there is no record of when they actually landed, and a wrong
 * date invented here would make a fresh download look decades stale the moment this ships.
 */
...
/**
 * Where the listener got to in an episode.
 *
 * A new table rather than columns on `tracks`: an episode is not necessarily in the library at
 * all — it is usually a search result played straight out of the queue — so there is no row on
 * `tracks` to hang a position off.
 */
...
/**
 * Adds the table that lets a recap outlive the plays it was computed from.
 *
 * New table only — nothing existing is touched, so there is nothing here that can fail on a
 * populated database.
 */
