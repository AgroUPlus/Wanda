...
internal sealed class AgroAuthError(message: String, cause: Throwable? = null) :
    ...
    internal companion object {
         ...
         *
         * 401 is overloaded on the login endpoint: it answers both "those credentials were not
         * accepted" and "this account is not active yet". They need different words in front of
         * the user, and the status alone cannot separate them, so the message is consulted for
         * that one distinction and nothing else.
         */
...
internal fun AgroAuthError.explain(): String = when (this) {
    is AgroAuthError.Rejected ->
        "That username and passphrase were not accepted. Note the passphrase is not the same as a " +
            "device token — use the one the server showed you when the account was made."
