    ...
    item(key = "sources") {
        GroupedCard(modifier = Modifier.scale(rememberShelfEntranceScale(0))) {
            SettingsRow(
                ...
                // point — the user just needs telling it's ready, not how it got that way.
                subtitle = when {
            ...
            SettingsRow(
                ...
                subtitle = if (state.youTube) {
                    // A Google account can hold several YouTube channels and only the session
                    // knows which one is active, so naming it is the only way to be sure the
                    // right one is signed in. Falls back to the bare state while unknown.
            ...
            SettingsRow(
                ...
                // A phone's audio is not all music: ringtones, podcast downloads and voice memos
                // all satisfy MediaStore's IS_MUSIC. Narrowing the scan is set once and forgotten,
                // so it sits behind a long press rather than taking a row from the everyday action.
