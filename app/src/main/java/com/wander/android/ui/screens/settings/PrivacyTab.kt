...
internal fun LazyListScope.privacyTab(
    ...
    if (state.agroPaired && state.agroVisibility != null) {
        ...
        item(key = "visibility") {
            GroupedCard(modifier = Modifier.scale(rememberShelfEntranceScale(1))) {
                // Greyed out rather than hidden while state.incognito is on. The stored
                // preferences are left exactly as they were and come back untouched when it goes
                // off — state.incognito overrides them for as long as it is on, it does not
                // rewrite them. Hiding the rows instead would leave no way to tell what will be
                // shared again afterwards.
                if (state.incognito) {
                    SettingsRow(
    ...
    item(key = "forget") {
        GroupedCard(modifier = Modifier.scale(rememberShelfEntranceScale(3))) {
            SettingsRow(
